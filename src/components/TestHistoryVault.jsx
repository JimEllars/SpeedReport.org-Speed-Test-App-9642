...
const { FiArchive, FiTrash2, FiTrendingUp } = FiIcons;
...
export default function TestHistoryVault({ history, onClear }) {
  const maximum = Math.max(...history.map((test) => test.metrics.download), 1);
        ...
        <>
          <div className="history-chart">
            <div className="chart-label">
              <SafeIcon icon={FiTrendingUp} /> Download trend
            </div>
            <div className="bars">
              {history.slice(0, 10).reverse().map((test) => (
                <div className="bar-column" key={`bar-${test.id}`}>
                  <div
                    className="bar"
                    style={{
                      height: `${getBarHeight(test.metrics.download, maximum)}%`
                    }}
